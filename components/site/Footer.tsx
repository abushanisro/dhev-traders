...
export function Footer() {
  return (
    <footer className="relative overflow-hidden text-white" style={{ background: '#05100a' }}>
      ...
      {/* Content */}
      <div className="relative z-10 container-prose pt-16 pb-10">
        <div className="grid md:grid-cols-4 gap-10 pb-10">
          ...
          <nav aria-label="Footer navigation">
            ...
            <ul className="space-y-2.5 text-sm text-white/80">
              {quickLinks.map((l) => (
                <li key={l.h}>
                  <a href={l.h} className="transition-colors hover:text-gold">{l.l}</a>
                </li>
              ))}
            </ul>
          </nav>
          ...
          <address className="not-italic">
            ...
            <ul className="space-y-3 text-sm text-white/80">
              ...
              <li className="flex gap-3 mt-1">
                ...
                <span className="text-xs tracking-wide text-white/75">33ATPPG1745E1ZW</span>
              </li>
            </ul>
          </address>
        </div>
        ...
        {/* Divider + copyright */}
        <div className="border-t border-white/20 pt-5 flex flex-col md:flex-row items-center md:justify-between gap-1 md:gap-2 text-xs font-semibold text-white/60 text-center md:text-left">
          <div>© {new Date().getFullYear()} Dhev Traders. All rights reserved.</div>
          <div>Trusted FMCG Distributor in Villupuram, Tamil Nadu</div>
        </div>
      </div>
